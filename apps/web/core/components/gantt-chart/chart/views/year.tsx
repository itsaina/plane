...
import type { IYearMonthBlock } from "../../views";
import { groupMonthsToYears } from "../../views";

export const YearChartView = observer(function YearChartView(_props: any) {
  const { currentViewData, renderView } = useTimeLineChartStore();
  const monthBlocks: IMonthBlock[] = renderView;
  const yearBlocks: IYearMonthBlock[] = groupMonthsToYears(monthBlocks);
          ...
          >
            ...
            >
              ...
              <div className="flex h-5 w-full">
                {yearBlock.children.map((monthBlock, index) => (
                  <div
                    key={`sub-${rootIndex}-${index}`}
                    className={cn(
